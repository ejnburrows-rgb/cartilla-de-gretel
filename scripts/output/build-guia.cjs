...
const teacherDataRegex = /(\d+):\s*\{\s*evaluationPage:\s*(null|\d+),\s*rhymeTitle:\s*(null|".*?")\s*\}/g;
const teacherData = {};
let tMatch;
...
content = content.replace(/Teacher’s GuideObjectivesListening \/ Speaking \/ Viewing/g, "Teacher’s Guide\nObjectives\nListening / Speaking / Viewing\n");
...
content = content.replace(/Lesson DevelopmentOral Activity/g, "\nLesson Development\nOral Activity\n");
...
for (const part of parts) {
  ...
  const objLines = objectivesStr.split('\n').map(l => l.trim()).filter(l => l.length > 0);
  const headingsToSkip = ['Listening / Speaking / Viewing', 'Reading / Literature', 'Writing / Language Mechanics', 'Culture'];
  const objectives = [];
  ...
  // Extract Motivacion
  let motivation = '';
  const motMatch = lessonDevStr.match(/Motivación\n([\s\S]*?)(?=\nReading \/ Literature|\nOpen to Activity Book|\nWriting \/ Language Mechanics|$)/);
  if (motMatch) {
