...
for (let i = 1; i <= 24; i++) {
  ...
  if (!json.evaluationRef) {
    ...
    if (json.evaluationRef.page !== null && typeof json.evaluationRef.page !== "number")
      fileErrors.push("evaluationRef.page must be null or number");
    if (json.evaluationRef.note !== null && typeof json.evaluationRef.note !== "string")
      fileErrors.push("evaluationRef.note must be null or string");
  }

  if (!json.rhyme) {
    fileErrors.push("rhyme is missing");
  } else {
    if (json.rhyme.title !== null && typeof json.rhyme.title !== "string")
      fileErrors.push("rhyme.title must be null or string");
    if (json.rhyme.text !== null && typeof json.rhyme.text !== "string")
      fileErrors.push("rhyme.text must be null or string");
  }

  if (!json.provenance) {
    fileErrors.push("provenance is missing");
  } else {
    if (typeof json.provenance.source !== "string")
      fileErrors.push("provenance.source must be string");
    if (typeof json.provenance.verified !== "boolean")
      fileErrors.push("provenance.verified must be boolean");
  }
...
for (let i = 1; i <= 24; i++) {
  ...
  if (st === "missing_content") {
    ...
    out +=
      "  -> Paths Searched: docs/Transcripción Integral_ La cartilla de Gretel - Guía del profesor.txt, src/content/guides/lesson-*.tsx, src/data/**, teacher-folder-data.ts\n";
  }
}
