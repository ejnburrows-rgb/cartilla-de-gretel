...
if (bindingsRaw) {
	try {
		...
		if (keys.length !== 90) errors.push(`page-bindings.json has ${keys.length} entries, expected 90`);
		for (let p = 1; p <= 90; p++) {
...
if (seedRaw) {
	...
	for (const phrase of required) {
		if (!seedRaw.includes(phrase)) errors.push(`exercise-seed.ts missing locked instruction phrase: "${phrase}""`);
	}
}

// 4. Banned words check across the user-facing content
const BANNED = ["demo", "Demo", "DEMO", "Pixar", "pixar"];
for (const file of ["src/content/exercise-seed.ts", "src/content/lesson-meta.ts", "src/content/sentence-bank.ts"]) {
	const raw = read(file);
	if (!raw) continue;
...
console.log("\u2705  content validation passed: 24 lessons, 90 pages, locked Spanish instructions, no banned words.");
process.exit(0);
