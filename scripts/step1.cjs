...
const flipchartPages = fs.existsSync(flipchartDir) ? fs.readdirSync(flipchartDir).filter(f => f.endsWith('.jpg')) : [];
const workbookPages = [];
if (fs.existsSync(sourceDir)) {
  ...
  letters.forEach(l => {
    ...
    if (fs.statSync(p).isDirectory()) {
      fs.readdirSync(p).filter(f => f.endsWith('.jpg')).forEach(f => {
...
fs.writeFileSync(path.join(__dirname, '../src/data/page-inventory.json'), JSON.stringify(inventory, null, 2));
