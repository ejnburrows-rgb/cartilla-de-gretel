...
function normalizeId(raw) {
  ...
  const idDashed =
    idNoDash.length === 32
      ? idNoDash.slice(0, 8) + '-' + idNoDash.slice(8, 12) + '-' + idNoDash.slice(12, 16) + '-' + idNoDash.slice(16, 20) + '-' + idNoDash.slice(20)
      : stripped;
...
async function chunkCandidates(idDashed, diag) {
  try {
    ...
    diag.chunkBlockCount = Object.keys((data && data.recordMap && data.recordMap.block) || {}).length;
    const out = [];
...
function writeDiagnostic() {
  ...
  for (const t of diagTargets) {
    ...
    lines.push('#   candidates: ' + t.candidateCount + ', filtered: ' + t.filteredCount + (t.usedFallback ? ' (used fallback: all)' : ''));
    ...
    for (const c of t.candidates || []) {
      lines.push('#   - [' + (c.matched ? 'MATCH' : '     ') + '] block=' + (c.blockId ? c.blockId.slice(0, 8) : 'none    ') + ' uuidInUrl=' + (c.urlContainsUuid ? 'Y' : 'N') + ' src=' + c.source + ' :: ' + c.url);
    }
  }
  lines.push('');
  try {
