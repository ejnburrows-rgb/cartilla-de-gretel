...
wi.interactions.forEach(i => {
  if (i.transcriptionStatus === 'needs-review' && i.sourceStatus !== 'missing-source' && i.sourcePage) {
...
wi.interactions.forEach(i => {
  ...
  if (i.sourceStatus === 'missing-source' || i.sourceStatus === 'missing') lessonStats[l].missingSource++;
});
...
console.log('Fully verified lessons:', fullyVerified.map(l => 'L'+l.padStart(2,'0')).join(', '));
console.log('Lessons needing missing source images:', needMissingSource.map(l => 'L'+l.padStart(2,'0')).join(', '));
