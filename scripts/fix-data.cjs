  ...
  if ([18, 19, 20].includes(i.lessonNumber)) {
    ...
    if (i.id.includes("-pX-")) {
      ...
      if (i.id.includes("letter-tracing") || i.id.includes("word-reveal")) pageIdx = 0;
        ...
        i.id.includes("word-bank") ||
        i.id.includes("mini-story") ||
        i.id.includes("syllable-tap")
      )
        pageIdx = 2;
...
tg.lessons.forEach((l) => {
  if ([21, 22, 23, 24].includes(l.lesson)) {
    ...
    if (!l.procedures)
      l.procedures = "Administrar la evaluación a los estudiantes. Registrar los resultados.";
    l.needsReview = false;
  }
});
