...
for (const file of allValidArtFiles) {
  ...
  const name = parsed.name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, "");
...
for (const p of manifest.pages) {
  ...
  if (fs.existsSync(faithfulDir)) {
    const dirs = fs.readdirSync(faithfulDir).filter(d => fs.statSync(path.join(faithfulDir, d)).isDirectory());
    const match = dirs.find(d => d.startsWith(`leccion-${p.lesson}-`) || d === `leccion-${p.lesson}`);
    if (match) {
  ...
  for (const obj of p.objects) {
    if (obj.type === 'illustration') {
      ...
      if (!obj.asset || isBroken) {
        const wordNorm = obj.word.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, "");
