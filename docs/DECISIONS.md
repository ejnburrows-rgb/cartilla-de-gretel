...
- **2026-07-30 — Security, CI and child-data guardrails prepared on a branch;
  nothing touching real records was applied.** The class-code flow returned a
  child's `student_code` — a reusable credential — to an anonymous caller, so a
  join code alone was enough to list the children in a class and then read or
  write any one child's progress. The replacement issues a short-lived,
  single-child, single-class session token with server-side validation and one
  generic Spanish error for every failure, so the response cannot be used to
  confirm whether a class or a child exists; the client refuses a payload that
  still contains a reusable credential, which is what the negative tests assert.
  Three further guardrails ride along: public sign-up no longer implies teacher
  access (invitation or pending approval, role assignment stays server-side, and
  the access gate now has explicit pending / invalid / expired / unauthorized /
  loading / retry states that fail closed); `progress_events.meta` moves from
  arbitrary JSON on a named child to an allowlist that rejects unknown keys, with
  teacher notes capped and a plain-Spanish warning against diagnoses, medical or
  family detail; and one pull-request workflow now runs install, validators,
  typecheck, lint, unit tests, build, Playwright with synthetic fixtures, a
  secret scan and a bundle budget, so a failing test can no longer produce a
  green result. Performance: the unconditional full-book PDF preload is gone and
  the `manualChunks` rules — seven of eight of which pointed at directories
  (`maestro`, `alumno`, `familia`, `binder`) that do not exist in this repo, so
  the teacher lane was never actually split out — now target the real
  `routes/cartilla/teacher`, `imprimir` and `presentar` paths. Deliberately NOT
  done: migrations were written with matching rollbacks but not applied, no class
  code was rotated (`GRETEL`, `NOVO26` and every other real code untouched), no
  real student record was read or changed, and no retention purge was scheduled —
  the proposed 365-day default is configuration awaiting the owner's approval.
  Also unverified rather than claimed: the sandbox had no network access to clone
  the repo, so `pnpm typecheck/lint/test/build/test:e2e`, the validators, the
  secret scan, the responsive screenshots, the accessibility runs and the
  before/after bundle byte totals were not executed locally — they run on the new
  workflow against the pull request, and the pull request says so rather than
  reporting results that were never produced. Content untouched throughout:
