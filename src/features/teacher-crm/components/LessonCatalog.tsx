...
export function LessonCatalog() {
  ...
  const LessonCard = ({ l }: { l: (typeof vowels)[0] }) => (
    <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-stone-200/60 hover:-translate-y-1 hover:shadow-xl transition-all group flex flex-col items-center text-center relative overflow-hidden">
      <div
        className={`absolute -top-10 -right-10 w-24 h-24 rounded-full opacity-10 transition-transform group-hover:scale-150 ${l.type === "vowel" ? "bg-[#ea580c]" : "bg-[#0284c7]"}`}
      />
      ...
      <div
        className={`w-16 h-16 rounded-2xl flex items-center justify-center text-4xl font-black text-white shadow-inner mb-4 z-10 font-fredoka ${l.type === "vowel" ? "bg-gradient-to-br from-[#ea580c] to-[#c2410c]" : "bg-gradient-to-br from-[#0284c7] to-[#0369a1]"}`}
      >
        {l.focus}
      </div>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {filteredVowels.length > 0 && (
          <section className="mb-16">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
              {filteredVowels.map((l) => (
                <LessonCard key={l.lesson} l={l} />
              ))}
            </div>
          </section>
        )}
        ...
        {filteredVowels.length === 0 && filteredConsonants.length === 0 && (
          <div className="text-center py-20">
            ...
            <h3 className="text-2xl font-bold text-stone-500 font-fredoka">
