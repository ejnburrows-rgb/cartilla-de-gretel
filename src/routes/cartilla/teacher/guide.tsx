...
function TeacherGuide() {
  ...
  return (
    <div
      className="flex h-screen bg-stone-50 overflow-hidden cartilla-crm-theme"
      style={
        theme
          ? ({
    ...
    >
      ...
      <div className="w-80 flex-shrink-0 bg-white border-r border-stone-200 flex flex-col shadow-sm z-10 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          ...
          <div>
            <h2 className="text-xs font-black uppercase tracking-widest text-stone-400 mb-3 ml-2">
              Las 5 Vocales
            </h2>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-left">
                      <div
                        className={`font-bold ${isSelected ? "text-stone-900" : "text-stone-600"}`}
                      >
                        {v.title}
                      </div>
                    </div>
      ...
      <div className="flex-1 overflow-y-auto bg-stone-50/50">
        <div className="max-w-4xl mx-auto p-8 lg:p-12">
          {selectedCatalogEntry && theme && (
            <div className="mb-12">
              <div
                className="inline-flex items-center justify-center w-16 h-16 rounded-2xl text-3xl font-black text-white shadow-lg mb-6 font-fredoka"
                style={{ backgroundColor: theme.accent }}
              >
                {selectedCatalogEntry.title.replace("Vocal ", "").replace("Consonante ", "")}
              </div>
              <h2 className="text-4xl font-black text-stone-800 font-fredoka mb-4">
                {selectedCatalogEntry.title}
              </h2>
              <p
                className="text-lg text-stone-600 font-medium border-l-4 pl-4"
                style={{ borderColor: theme.accent }}
              >
                Sigue las siguientes instrucciones para guiar a los estudiantes en las actividades
                de esta lección.
              </p>
            </div>
          )}
          ...
          {selectedData.length === 0 ? (
            <div className="text-center p-12 bg-white rounded-3xl border border-stone-200">
              <p className="text-stone-500 font-bold">
                No hay notas extraídas para esta lección todavía.
              </p>
            </div>
          ) : (
            <div className="space-y-8">
              {selectedData.map((exercise, idx) => (
                <div
                  key={exercise.id}
                  className="bg-white rounded-3xl p-8 shadow-sm border border-stone-200 relative overflow-hidden group"
                >
                  <div
                    className="absolute top-0 left-0 w-2 h-full"
                    style={{ backgroundColor: theme?.accent || "#ccc" }}
                  />
