...
export const Route = createFileRoute("/cartilla/recursos/guia/$n")({
  ...
  beforeLoad: ({ params }) => {
    ...
    if (!Number.isFinite(n) || !CATALOG.find((e) => e.n === n)) {
      throw redirect({ to: "/cartilla/recursos" });
    }
  },
});
...
function TeacherGuideLeccion() {
  ...
  const handleSelectLesson = (newN: number) => {
    navigate({ to: "/cartilla/recursos/guia/$n", params: { n: String(newN) } });
  };
