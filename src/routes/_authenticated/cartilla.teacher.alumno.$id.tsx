...
function StudentDetail() {
  ...
  const summary = useMemo(() => {
    ...
    for (const e of data.events) {
      ...
      if (e.event_kind === "badge") badges.push({ name: String((e.meta as Record<string, unknown> | null)?.name ?? "Insignia"), at: e.created_at });
      if (e.event_kind === "level" && !level) level = { value: String((e.meta as Record<string, unknown> | null)?.level ?? "—"), at: e.created_at };
    }
  ...
  if (error) {
    return (
      <main className="min-h-screen bg-background px-4 py-6 max-w-4xl mx-auto">
        ...
        <div className="kid-card mt-6 border-destructive/20 bg-destructive/5 p-6 text-sm font-bold text-destructive">No se pudo cargar el progreso del alumno. Revisa la sesión del maestro o la conexión.</div>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-background px-4 py-6 max-w-4xl mx-auto">
      ...
      <section className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-3">
        <StatCard icon={BookOpen} label="Lecciones" value={`${summary.completedCount}/${TOTAL_LESSONS}`} />
        <StatCard icon={Target} label="Ejercicios" value={String(Object.values(summary.exerciseStats).reduce((a, s) => a + s.runs, 0))} />
      ...
      <section className="mt-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {CATALOG.map((entry) => {
            ...
            return (
              <div key={entry.n} className="kid-card p-3 flex items-center gap-3 border-l-4" style={lessonBorderStyle(entry.color)}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-foreground/60 mt-0.5">
                    {isDone ? <span className="text-success font-bold">✓ completada</span> : <span>pendiente</span>}
                    {ex && <span className="ml-2">· {ex.runs} ejercicio{ex.runs > 1 ? "s" : ""} ({pct}% acierto)</span>}
                  </div>
                </div>
              </div>
            );
      ...
      {summary.badges.length > 0 && <section className="mt-8"><h2 className="font-bold mb-3 text-lg">Insignias ganadas</h2><ul className="flex flex-wrap gap-2">{summary.badges.map((b, i) => <li key={i} className="kid-card p-2 px-3 inline-flex items-center gap-2 text-sm"><Award className="w-4 h-4 text-primary" /> {b.name}</li>)}</ul></section>}

      <section className="mt-8">
        <h2 className="font-bold mb-3 text-lg">Actividad reciente</h2>
        <ul className="space-y-1.5 text-sm">
          {data.events.slice(0, 30).map((e) => <li key={e.id} className="flex items-center justify-between gap-3 px-3 py-2 rounded-lg bg-card border border-foreground/5"><span><span className="font-bold text-foreground/70">L{e.lesson_id}</span> <span className="text-foreground/60">{labelEvent(e)}</span></span><span className="text-xs text-foreground/50">{new Date(e.created_at).toLocaleString()}</span></li>)}
          {data.events.length === 0 && <li className="text-foreground/60">Sin actividad todavía.</li>}
        </ul>
      </section>
    </main>
  );
...
function lessonBorderStyle(color: string) {
  return { borderLeftColor: color };
}

function labelEvent(e: { event_kind: string; score: number | null; total: number | null; time_seconds: number | null }) {
  switch (e.event_kind) {
    case "lesson_completed": return "lección completada";
    case "exercise": return `ejercicio ${e.score ?? 0}/${e.total ?? 0}`;
    case "time": return `${e.time_seconds ?? 0} s de estudio`;
    case "badge": return "insignia ganada";
    case "level": return "cambio de nivel";
    default: return e.event_kind;
  }
}

function StatCard({ icon: Icon, label, value }: { icon: typeof BookOpen; label: string; value: string }) {
  return <div className="kid-card p-3 flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-primary/10 text-primary flex items-center justify-center"><Icon className="w-5 h-5" /></div><div className="min-w-0"><div className="text-xs text-foreground/60">{label}</div><div className="font-bold truncate">{value}</div></div></div>;
}
