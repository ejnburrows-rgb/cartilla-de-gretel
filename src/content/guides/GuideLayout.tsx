...
export function GuideLayout({ children, selectedLesson, onSelectLesson, accentColor = "#f97316" }: GuideLayoutProps) {
          ...
          {CATALOG.map((entry) => (
            ...
            >
              <div className="flex flex-col">
                <span className={`text-xs font-black uppercase tracking-wider ${selectedLesson === entry.n ? "text-stone-300" : "text-stone-400"}`}>
                  Lección {entry.n}
                </span>
                <span className={`text-sm font-bold truncate pr-2 ${selectedLesson === entry.n ? "text-white" : "text-stone-700"}`}>
                  {entry.title}
                </span>
              </div>
              <ChevronRight className={`w-4 h-4 shrink-0 ${selectedLesson === entry.n ? "opacity-100" : "opacity-0"}`} />
            </button>
          ))}
        </div>
      </aside>
        ...
        <div className="flex items-center gap-2 p-3 bg-stone-50/80 border-b border-stone-200/60 overflow-x-auto no-scrollbar">
          <button 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold text-sm transition-colors whitespace-nowrap ${
              activeTab === "objetivos" ? "bg-white text-stone-800 shadow-sm border border-stone-200" : "text-stone-500 hover:text-stone-700 hover:bg-stone-100"
            }`}
          >
            <Target className={`w-4 h-4 ${activeTab === "objetivos" ? "text-orange-500" : "text-stone-400"}`} />
            Objetivos
          </button>
          ...
          <button 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold text-sm transition-colors whitespace-nowrap ${
              activeTab === "procedimiento" ? "bg-white text-stone-800 shadow-sm border border-stone-200" : "text-stone-500 hover:text-stone-700 hover:bg-stone-100"
            }`}
          >
            <ListTodo className={`w-4 h-4 ${activeTab === "procedimiento" ? "text-emerald-500" : "text-stone-400"}`} />
            Procedimiento
          </button>
          ...
          <button 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold text-sm transition-colors whitespace-nowrap ${
              activeTab === "vocabulario" ? "bg-white text-stone-800 shadow-sm border border-stone-200" : "text-stone-500 hover:text-stone-700 hover:bg-stone-100"
            }`}
          >
            <BookOpen className={`w-4 h-4 ${activeTab === "vocabulario" ? "text-blue-500" : "text-stone-400"}`} />
            Vocabulario y Poema
          </button>
          ...
          <button 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold text-sm transition-colors whitespace-nowrap ${
              activeTab === "evaluacion" ? "bg-white text-stone-800 shadow-sm border border-stone-200" : "text-stone-500 hover:text-stone-700 hover:bg-stone-100"
            }`}
          >
            <GraduationCap className={`w-4 h-4 ${activeTab === "evaluacion" ? "text-purple-500" : "text-stone-400"}`} />
            Evaluación
          </button>
        </div>
