...
/**
 * Legacy name kept so existing games do not need a risky cross-app refactor.
 * The sound itself is intentionally NOT a buzzer: two quiet descending piano
 * notes signal "try again" without a harsh failure cue.
 */
export function playWrongBuzz() {
  if (typeof window === "undefined" || audioEngine.isMuted()) return;
  playNote(293.66, 0.24);
  window.setTimeout(() => playNote(261.63, 0.28), 90);
}
