...
export const BOOK_FAITHFUL_LESSONS: ReadonlyArray<BookFaithfulLesson> = raw.bookFaithfulLessons;

export const SIGHT_WORD_INDEX: Readonly<Record<string, SightWordEntry>> = raw.sightWordIndex;

export const MINI_STORY_LESSONS: ReadonlyArray<number> = raw.miniStoryLessons;
export const EMPTY_PALABRAS_LESSONS: ReadonlyArray<number> = raw.emptyPalabrasLessons;
...
export function getBookFaithfulLesson(lessonNumber: number): BookFaithfulLesson | null {
  return raw.bookFaithfulLessons.find((l) => l.lesson === lessonNumber) ?? null;
}
...
export function getEditorialNotesForLesson(lessonNumber: number): EditorialNote[] {
  const needle = `Lesson ${lessonNumber}`;
  return raw.editorialNotes.filter((n) => n.location.toLowerCase().includes(needle.toLowerCase()));
}
...
export function getFullWorkbookPages(): Array<{
  page: number;
  lesson: number;
  imageScanReference: string | null;
  section: "Introducción" | "Vocales" | "Consonantes";
}> {
  const out = [];
  for (const entry of CATALOG) {
    ...
    const section = (
      sectionRaw === "intro" ? "Introducción" : sectionRaw === "vowels" ? "Vocales" : "Consonantes"
    ) as "Introducción" | "Vocales" | "Consonantes";
