...
export type FlipchartDeliveryTier = "canonical" | "screen" | "thumb";
...
/**
 * Return the canonical HD master URL for a flipchart page (leading slash).
 * This is tier 2: faithful cleaned/restored source, never a thumbnail.
 */
...
/**
 * Tier-3 app delivery URLs generated by scripts/build-delivery-assets.mjs.
 * Main projector surfaces get a retina-friendly 1920px WebP; filmstrip cards
 * get a small 360px WebP. Canonical masters remain immutable and addressable.
 */
export function getFlipchartDeliverySrc(
  page: FlipchartPage,
  tier: FlipchartDeliveryTier = "screen",
): string {
  if (tier === "canonical") return getFlipchartPageSrc(page);
  const n = String(page.flipchartPage).padStart(3, "0");
  return `/cartilla/art/delivery/flipchart/${tier}/page-${n}.webp`;
}
