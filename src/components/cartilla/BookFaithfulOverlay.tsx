...
export function BookFaithfulOverlay({ n }: { n: number }) {
  ...
  const hasAny = sightWords.length > 0 || miniStory || emptyPalabras || notes.length > 0;
  if (!hasAny) return null;

  return (
    <aside className="mt-3 flex flex-col gap-2" aria-label="Información de la lección">
      {sightWords.length > 0 && (
        <div
            ...
            return (
              ...
              >
                ...
                <div>
                  ...
                  <div className="text-[10px] opacity-70 mt-0.5 font-medium">{note.location}</div>
                </div>
              </li>
            );
          })}
