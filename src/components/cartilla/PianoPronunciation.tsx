...
export function PianoPronunciation({
  ...
  useEffect(() => {
    ...
    if (matchIdx !== -1) {
      ...
      // Log progress only inside a real student lesson context. Public family
      // practice intentionally omits lessonId so it can never attribute data
      // to a student session that happens to exist in the same browser.
      if (lessonId) {
        recordEvent({
          lessonId,
          kind: "exercise",
