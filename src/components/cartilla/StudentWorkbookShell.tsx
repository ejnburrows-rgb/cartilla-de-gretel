      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-7xl flex-col px-3 py-4 sm:px-5 sm:py-6">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2 text-[#3A281E]">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Link to="/cartilla/lecciones" className="inline-flex min-h-10 items-center gap-2 rounded-full border border-white/55 bg-white/55 px-3 py-2 text-xs font-black shadow-sm backdrop-blur hover:bg-white/80">
            ...
            {pageNumbers.length > 0 && (
              <div className="mt-4 flex flex-wrap gap-1.5" aria-label="Páginas del libro">
                {pageNumbers.map((page) => (
                  <span key={page} className="grid h-9 min-w-9 place-items-center rounded-lg border border-white/65 bg-white/85 px-2 text-sm font-black text-[var(--cartilla-title-ink)] shadow-sm">{page}</span>
                ))}
              </div>
            )}
          </header>
