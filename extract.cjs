...
const fs = require('fs');
if (!fs.existsSync('remote_status.html')) { console.log('not ready'); process.exit(0); }
...
console.log('\n--- Blocker Cards ---');
const blockerRegex = /<h3[^>]*>Bloqueo:([^<]+)<\/h3>[\s\S]*?<div class="blocker-meta">([\s\S]*?)<\/div>[\s\S]*?<p[^>]*>([\s\S]*?)<\/p>/g;
while ((match = blockerRegex.exec(html)) !== null) {
  console.log('Blocker:', match[1].trim());
  console.log('Meta:', match[2].replace(/<[^>]+>/g, '').replace(/\s+/g, ' ').trim());
  console.log('Desc:', match[3].replace(/<[^>]+>/g, '').replace(/\s+/g, ' ').trim());
  console.log('-');
}
...
while ((match = rowRegex.exec(html)) !== null) {
  ...
  if (status.includes('needs-source-verification') || status.includes('scaffold') || (fidelity < 80)) {
