...
**Real finding, not a gap**: `mono` and `luna` were checked directly against
both the workbook scans and the corresponding flipchart pages for their
lessons — neither word has any illustration anywhere in this book edition,
same situation as the already-documented `moto`/`mapa`. Left as honest "art
pending," not re-crop candidates. The remaining 33-word backlog (down from
39) plus a ready-to-paste Antigravity prompt is in `ART_BACKLOG.md`.
